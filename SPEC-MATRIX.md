...
| §9.3.1 inspection steps 4 and 5 | §9.3.1 | 🔴 two of the five mandated issuer-data-authentication steps are missing. **Step 4** — "verify that the DocType in the MSO matches the relevant DocType in the Documents structure": never compared (`MdocReader.kt:86-101`, `MdocReader.swift:108`); `DeviceAuthentication` is signed over the *Document's* docType while the app is handed the *MSO's*. **Step 5, first bullet** — "the 'signed' date is within the validity period of the certificate in the MSO header": `X509ChainValidator` always validates at `now()` (`X509ChainValidator.kt:48`), so an otherwise-valid mdoc whose DS certificate has since expired is rejected, while an MSO signed before the DS `notBefore` passes. `validFrom`/`validUntil` (bullets 2-3) *are* checked (`MdocVerifier.kt:54-56`) |
