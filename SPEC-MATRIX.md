...
| BLE / NFC transport production hardening | 🟡 demo adapters + live Multipaz interop done; **timeout + cancellation/failure cleanup done, BLE Ident added (device-verified)**; MTU is negotiated; **automatic reconnect still pending** (needs a transport state-machine refactor) |
