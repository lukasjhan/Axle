...
| HAIP | **1.0 Final** | ✅ **발급 프로파일 완료 (2026-07-05) 양 언어** — PAR·DPoP·PKCE Required 기본, wallet attestation client 인증, key attestation, batch, deferred, notification, refresh-token reissuance, signed metadata policy (아래 행들 참조) |
...
| Reissuance + signed metadata | refresh-token 재발급(RFC 6749 §6), signed issuer metadata 정책(OpenID4VCI §11.2.3) | ✅ **완료 (2026-07-05) 양 언어** — `TokenResponse.refreshToken`, `CredentialResponse.canReissue`(+refreshToken·configurationId 컨텍스트), `reissue(previous, keys)`가 refresh_token 그랜트→새 access token→requestCredential(키 로테이션 가능). `IssuerMetadataPolicy`(IgnoreSigned/PreferSigned/RequireSigned) + `SignedMetadataVerifier` 포트(앱이 trust로 x5c 검증), loadIssuerMetadata가 `signed_metadata` JWS 검증→verified 클레임 overlay. 테스트: 재발급 라운드트립, RequireSigned가 verified 클레임 사용/미서명 거부/잘못된 서명 거부. **(주의: `= runBlocking{...}` 마지막 표현식이 assertFailsWith면 non-Unit 반환→JUnit이 조용히 스킵. `runBlocking<Unit>` 사용. 코드베이스 전체 스캔으로 다른 스킵 없음 확인)** |
