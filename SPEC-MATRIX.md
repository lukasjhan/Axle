...
| `transaction_data` | §8.4/B.3.3 | ✅ SD-JWT VC — each entry is parsed and bound (a `sha-256` `transaction_data_hashes` value in the KB-JWT) to exactly one of its `credential_ids` (§5.1). Rejected with `invalid_transaction_data`: malformed entries, unknown `credential_ids`, a referenced query with `require_cryptographic_holder_binding=false` (B.3.3), a hash-alg set without `sha-256`, and (when configured) unsupported `type`s. 🟡 mdoc `transaction_data` (B.2.1) still not carried — an entry bindable only to a presented mdoc is rejected |
