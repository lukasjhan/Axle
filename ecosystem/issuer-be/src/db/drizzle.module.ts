...
/**
 * Postgres (postgres.js) via Drizzle, configured from `DATABASE_URL`. The schema is applied with drizzle-kit
 * migrations (`drizzle.config.ts` → `drizzle/`, run by `src/migrate.ts` / `pnpm migrate`), so a fresh
 * deployment migrates before serving (e.g. an init container / Job in k8s).
 */
@Global()
@Module({
  providers: [
