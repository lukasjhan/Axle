...
| terminal | `Completed(redirectUri, dcApiResponse)` · `Declined` · `Failed(error)` | `.completed(redirectUri:, dcApiResponse:)` · `.declined` · `.failed(error)` |
...
when (done) {
    ...
    is PresentationState.Declined  -> { /* user refused */ }
...
for await state in session.states {
    ...
    case .declined: break                                  // user refused
